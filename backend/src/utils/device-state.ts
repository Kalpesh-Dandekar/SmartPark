import type { DeviceCommandStatus, StoredReservationStatus, SlotStatus } from "../types/index.js";
...
export function assertAuthorizationState(input: { requestingUserId: string; reservationUserId: string; reservationStatus: StoredReservationStatus; withinWindow: boolean; reservationId: string; reservationSlotId: string; slotId: string; slotStatus: SlotStatus; currentReservationId: string | null }) {
...
export function assertParkingState(input: { commandSlotId: string; eventSlotId: string; commandReservationId: string; reservationSlotId: string; reservationStatus: StoredReservationStatus; slotStatus: SlotStatus; currentReservationId: string | null }) {
...
export function assertVacancyState(input: { commandSlotId: string; eventSlotId: string; commandReservationId: string; reservationSlotId: string; reservationStatus: StoredReservationStatus; slotStatus: SlotStatus; currentReservationId: string | null }) {
  if (input.commandSlotId !== input.eventSlotId || input.reservationSlotId !== input.eventSlotId) throw new HttpError(409, "Command, reservation, and slot do not match", "SLOT_MISMATCH");
  if (input.reservationStatus !== "ACTIVE") throw new HttpError(409, "Reservation is not active", "INVALID_RESERVATION_STATE");
