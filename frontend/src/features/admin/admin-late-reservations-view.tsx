...
function ReservationTable({ items, busyId, onExpire }: { items: AdminReservation[]; busyId: string; onExpire: (item: AdminReservation) => void }) { if (!items.length) return <p className="p-8 text-center text-sm text-slate-600">No reservations match this view.</p>; return <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">User</th><th className="px-4 py-3">Vehicle</th><th className="px-4 py-3">Booking</th><th className="px-4 py-3">Duration</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Lifecycle time</th><th className="px-5 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{items.map((item) => <tr key={item.id} className="align-top"><td className="px-5 py-4"><p className="font-semibold">{item.userName}</p><p className="mt-1 max-w-56 break-all text-xs text-slate-500">{item.userEmail}</p></td><td className="px-4 py-4 font-mono font-semibold">{item.vehicleNumber}</td><td className="px-4 py-4"><p>{formatAdminDate(item.startAt ?? item.bookingDate)}</p><p className="mt-1 text-xs text-slate-500">{formatAdminTime(item.startAt)}–{formatAdminTime(item.endAt)}</p></td><td className="px-4 py-4">{item.durationMinutes} min</td><td className="px-4 py-4"><ReservationBadge status={item.status} /></td><td className="px-4 py-4 text-xs text-slate-600">{lifecycleTime(item)}</td><td className="px-5 py-4 text-right">{item.status === "BOOKED" ? <Button size="sm" variant="destructive" disabled={busyId === item.id} onClick={() => onExpire(item)}>{busyId === item.id ? "Expiring…" : "Expire Reservation"}</Button> : <span className="text-xs text-slate-400">No action</span>}</td></tr>)}</tbody></table></div>; }
...
function DevicePanel({ device }: { device: AdminDashboardData["device"] }) { return <section id="system-status" className="scroll-mt-6"><Card><CardHeader className="flex flex-row items-start gap-3 border-b border-slate-200"><RadioTower className="mt-0.5 size-5 text-slate-500" /><div><CardTitle>IoT device status</CardTitle><p className="mt-1 text-sm text-slate-600">Cloud telemetry is monitored separately from parking capacity.</p></div></CardHeader><CardContent className="p-5 sm:p-6">{device ? <dl className="grid gap-4 sm:grid-cols-2"><Info label="Recent device event" value={humanize(device.lastEventType)} /><Info label="Last activity" value={formatAdminDateTime(device.lastActivityAt)} /></dl> : <p className="text-sm text-slate-600">No recent IoT activity is available.</p>}</CardContent></Card></section>; }
